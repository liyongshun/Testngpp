...
TestNG++的运行时机制要求，每个测试套件源代码文件需要被编程和链接成一个 module 。在Linux下，一个 module 是一个所有符号都得到充分链接的*共享库(Shared Object File)；在Mac OS下，一个**module对应一个bundle，而在windows上，一个module则是一个动态链接库(Dynamic Link Library)。**
